...
struct state {
  ...
  static constexpr std::array<char, 16>      targets
      = {'A', 'A', 'A', 'A', 'B', 'B', 'B', 'B',
         'C', 'C', 'C', 'C', 'D', 'D', 'D', 'D'};
  static constexpr int room_size = 4;

  std::array<char, 11> hallway = {};
  std::array<char, 16> rooms   = {};
  int                  energy  = 0;
  ...
  static state load()
  {
    ...
    ret.rooms = {lines[2][3], lines[3][3], lines[4][3], lines[5][3],
      ...
      if (hallway[h] != '.') {
        ...
        if (rooms[r_max] == '.' && reachable_from(h, room_hall_entry(r_max))
          ...
          ret.push_back(next);
            ...
            rooms[r_max - 2] == '.'
            && reachable_from(h, room_hall_entry(r_max - 2))
            && room_exit_clear(r_max - 2) && rooms[r_max] == pod
            && rooms[r_max - 1] == pod) {
          auto next = *this;
          next.energy += room_hall_distance(r_max - 2, h) * costs.at(pod);
          std::swap(next.rooms[r_max - 2], next.hallway[h]);
          ret.push_back(next);
        } else if (
            rooms[r_max - 3] == '.'
            && reachable_from(h, room_hall_entry(r_max - 3))
            && room_exit_clear(r_max - 3) && rooms[r_max] == pod
            && rooms[r_max - 1] == pod && rooms[r_max - 2] == pod) {
          auto next = *this;
          next.energy += room_hall_distance(r_max - 3, h) * costs.at(pod);
          std::swap(next.rooms[r_max - 3], next.hallway[h]);
          ret.push_back(next);
        }
      }
    }
  ...
  int room_hall_entry(int room) const { return ((room / 4) + 1) * 2; }

  int room_hall_distance(int room, int hall) const
  {
    int exit_cost = (room % 4) + 1;
...
###{}#{}#{}#{}###
  ...
  #########)";
    ...
    return format_to(
        ctx.out(), str, fmt::join(s.hallway, ""), s.rooms[0], s.rooms[4],
        s.rooms[8], s.rooms[12], s.rooms[1], s.rooms[5], s.rooms[9],
        s.rooms[13], s.rooms[2], s.rooms[6], s.rooms[10], s.rooms[14],
        s.rooms[3], s.rooms[7], s.rooms[11], s.rooms[15]);
  }
};
